...
func (t Training) Calories() float64 {
	...
	/*	switch  t.TrainingType {
		case "Плавание": {
			return Swimming.Calories()
		}
		case "Ходьба": {
			return Walking.Calories()
		}
		case "Бег": {
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	return float64(CaloriesMeanSpeedMultiplier)*r.Training.meanSpeed() + CaloriesMeanSpeedShift
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	return (CaloriesWeightMultiplier*w.Training.Weight + (math.Pow(w.Training.meanSpeed()*KmHInMsec, 2)/(float64(w.Height)*CmInM))*CaloriesSpeedHeightMultiplier*w.Training.Weight) * w.Training.Duration.Hours() * float64(MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	return float64(s.LengthPool*s.CountPool) / float64(MInKm) / s.Training.Duration.Hours()
}
